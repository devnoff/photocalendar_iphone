...
@synthesize selectedTile, highlightedTile, transitioning,delegate;
...
- (id)initWithFrame:(CGRect)frame logic:(KalLogic *)theLogic delegate:(id<KalViewDelegate>)theDelegate
{
  ...
  if (self = [super initWithFrame:frame]) {
    ...
    backMonthView = [[KalMonthView alloc] initWithFrame:monthRect];
      frontMonthView.delegate = self;
      frontMonthView._dataSource = delegate;
      backMonthView.delegate = self;
      backMonthView._dataSource = delegate;
    ...
    self.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"Kal.bundle/kal_grid_background"]];
  return self;
}
...
- (void)receivedTouches:(NSSet *)touches withEvent:event
{
  ...
  if ([hitView.superview isKindOfClass:[KalTileView class]]) { // hitView 커스텀 KalDateView 이기때문에 superview 와 비교
