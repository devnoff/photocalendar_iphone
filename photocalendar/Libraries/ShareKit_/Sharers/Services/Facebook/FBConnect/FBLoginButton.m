...
- (void)touchUpInside {
  if (_session.isConnected) {
    ...
    FBLoginDialog* dialog = [[(FBLoginDialog *)[FBLoginDialog alloc] initWithSession:_session] autorelease];
    [dialog show];
  }
}
...
- (NSString *)accessibilityLabel {
	if (_session.isConnected) {
		return NSLocalizedStringFromTable(@"Disconnect from Facebook",@"LocalizableSHK", @"Accessibility label");
	} else {
		return NSLocalizedStringFromTable(@"Connect to Facebook",@"LocalizableSHK", @"Accessibility label");
	}
}
